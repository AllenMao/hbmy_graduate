package
{
	...
	public class main extends MovieClip
	{
		...
		public var channel:SoundChannel;//声音通道
		private var curPosition:Number=0;//播放当前位置
		private var index:int=1;//当前歌表索引
		...
		private var SongName:String;//当前歌名
		...
		public function main()
		{
			...
			bgImage=new Image();
			lrc=new songLrc();
			songName.wordWrap=true;
			lrcTxt.wordWrap=true;
			//lrcTxt.autoSize=true;
			
			musicList=["music/m1.mp3","music/m2.mp3","music/m3.mp3","music/m4.mp3","music/m5.mp3"];
			imageList=["pic/pic1.jpg","pic/pic2.jpg","pic/pic3.jpg","pic/pic4.jpg","pic/pic5.jpg"];
		...
		{
			SongName=String(index+1)+"-"+(snd.id3.songName);
			songName.text=String(index+1)+"-"+(snd.id3.songName);
		}
		...
		private function toTime(n:uint):String
		{
			...
			var m:uint=s/60;
			s%=60;
			return String(100+m).substr(1,2)+":"+String(100+s).substr(1,2);
		}
		...
		/*显示歌词*/
		function SoundPlaying(evt:Event)
		{
    		for (var i=1; i<lrc.LRCarray.length; i++)
    		{
				//trace("sd"+"   "+mainPoint.channel.position);
        		if (channel.position<lrc.LRCarray[i].timer)
        		{
					if(i>=2&&i<lrc.LRCarray.length)
					{
						lrcTxt.text=lrc.LRCarray[i-2].lyric+lrc.LRCarray[i-1].lyric+lrc.LRCarray[i].lyric;
					}
            		else lrcTxt.text=lrc.LRCarray[i-1].lyric;
					
            		break;
        		}
        		lrcTxt.text=lrc.LRCarray[lrc.LRCarray.length-1].lyric;
    		}
		}
	}
}