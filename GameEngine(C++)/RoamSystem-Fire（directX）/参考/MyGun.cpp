...
{
	if(FAILED(D3DXLoadMeshFromX("res/model/gun.x", D3DXMESH_MANAGED, Device, &pAdjacencyBuffer, &pMaterialsBuffer, NULL, &numMaterials, &xmesh)))
	{
	...
	D3DXCreateTextureFromFile(Device,"res/texture/muzzle.dds",&muzzle_tex);

}

void MyGun::Update(IDirect3DDevice9 *Device,CCamera *Camera,float time)
{
	...
	matEmitterWorld = matTrans * matMuzzleTrans  *matRot* matViewInverse;//枪口矩阵
