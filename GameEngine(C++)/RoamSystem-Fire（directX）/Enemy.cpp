...
{
	enemy->LoadFromXFile("enemy_xia.x");
	//angle = 2*D3DX_PI*(rand()%1000)/1000.0f;
	state = 0;
	HP = 48;
	pos = D3DXVECTOR3( 500.0f,Terrain->GetCameraHeight(500.0f,500.0f),500.0f );
	angle = 0; 
	walkspeed = 0.8f;
	Scale =0.9f;
...
void Enemy::Move(CCamera *Camera,CTerrain *Terrain)
{
	if(state !=2)
	{
		...
		pos.x += MoveX;
		pos.z += MoveZ;
		pos.y = Terrain->GetCameraHeight(pos.x,pos.z)+70.0f;
		min = m_Min +pos;
		max = m_Max +pos;
	}
}
...
void Enemy::Render(IDirect3DDevice9 *Device,float time)
{
	...
	if(state !=2)
	{
		...
		{
			Run();
			enemy->Render(3.0f*time,pos,angle+1.5,Scale);
		}
		else
		{
			Attack();
			enemy->Render(6.0f*time,pos,angle+1.5f,Scale);//ËÙ¶ÈÂÔ¿ì
		}
	}
	enemy->Render(2.7f*time,pos,angle+1.5f,Scale);
}

void Enemy::Attack()
{
	enemy->SetAnimationName("hit");//"fight"
	state =1;
}

void Enemy::Run()
{
	enemy->SetAnimationName("walk");//"run"
	state = 0;
}
...
void Enemy::Die()
{
	enemy->SetAnimationName("dead");//"fight_down"
	state =2;
}
